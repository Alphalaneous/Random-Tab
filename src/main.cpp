...
class $modify(MyLevelSearchLayer, LevelSearchLayer) {
	...
	bool init(int type) {
		...
		auto quickSearchMenu = getChildByIDRecursive("quick-search-menu");
		...
		auto oldSprite = static_cast<CCSprite*>(randomButtonSprite->getChildren()->objectAtIndex(1));
	...
	void onRandom(CCObject* obj){
		...
		for (int n = 0; n < 10000; n++){
			...
			query << utils::random::generate(128, 150000000);
			addSeparator = true;
		}
